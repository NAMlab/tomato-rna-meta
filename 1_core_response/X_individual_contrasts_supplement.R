...
# @TODO make this into a loop (very simple, just loop through trt.groups and skip controls like in diffexp script)
trt.group = "huji.1"
...
write.csv(out.csv, paste0("output/individual_contrasts_supplement/", trt.group, ".csv"), row.names=F, na = "")

#### GO terms ####
genes.up = unique(str_remove(out.csv[out.csv$FDR < 0.05 & out.csv$logFC > 0,]$target_id, "\\.[0-9]+\\.[0-9]+$"))
write.csv(go_enrichment(genes.up), paste0("output/individual_contrasts_supplement/", trt.group, "_GO_upregulated.csv"), row.names=F)
genes.down = unique(str_remove(out.csv[out.csv$FDR < 0.05 & out.csv$logFC < 0,]$target_id, "\\.[0-9]+\\.[0-9]+$"))
write.csv(go_enrichment(genes.down), paste0("output/individual_contrasts_supplement/", trt.group, "_GO_downregulated.csv"), row.names=F)
...
ggsave(paste0("output/individual_contrasts_supplement/", trt.group, ".jpg"), width = 30, height = 20, units = "cm")
